...
const MyPostsPage = () => {
    ...
    useEffect(() => {
        const getMyPosts = async () => {
            ...
            } = await request.get(`post/user?limit=${LIMIT}&${search ? `search=${search}` : ""}`);
    ...
    const refetchData = async () => {
        if (posts.length < total) {
            const {
                data: { data: data1, pagination },
            } = await request.get(`post/user?limit=${LIMIT}&page=${page}&${search ? `search=${search}` : ""}`);
        ...
        <div className="posts">
            <div className="container">
                ...
                <form className="posts__form">
                    <input type="text" id="name" placeholder="Searching ..." onChange={handleSearch} />
                ...
                <div className="posts__box">
                    ...
                    >
                        {loader ? <Loader /> : posts.map((post, i) => <MyPostCard key={i} {...post} deletePost={deletePost} editPost={editPost} />)}
                    </InfiniteScroll>
                </div>

                <Modal open={open} title="Add post" onOk={handleSubmit(submit)} onCancel={closeModal} okText={`${selected === null ? "Add" : "Save"} post`}>
                    {formLoad ? (
                        <Loader />
                    ) : (
