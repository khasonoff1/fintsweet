...
const PopularPostsCard = ({ _id, title, description, createdAt, user: { first_name, last_name }, photo }) => {
        ...
        <Link to={`/${_id}`}>
            <div className="postsCard">
                <div className="postsCard__img-box">
                    <LazyLoadImage effect="blur" className="postsCard__img" src={`${IMG_URL}${photo2}`} alt="" onError={errorHandler} />
                </div>
                <div className="postsCard__body">
                    <p className="postsCard__date">
                        By <span>{first_name + " " + last_name}</span> | {`${date[0]} ${date[1]}, ${date[2]}`}
                    </p>
